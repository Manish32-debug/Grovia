  ...
  try {
    ...
    if (!result.ok) {
      throw result.reason === 'expired'
        ? new AppError(401, ErrorCode.TOKEN_EXPIRED, 'Your session expired. Refreshing…')
    ...
    if (user.tokenVersion !== result.claims.tv) {
      throw new AppError(401, ErrorCode.SESSION_REVOKED, 'Your password changed. Sign in again.');
    }
...
export const optionalAuthenticate: RequestHandler = (req, res, next) => {
  ...
  authenticate(req, res, (err) => next(err instanceof AppError ? undefined : err));
};
